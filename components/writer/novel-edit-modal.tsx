...
export function NovelEditModal({ novel, open, onClose, onSave }: NovelEditModalProps) {
  ...
  const [draft, setDraft] = useState<WriterNovelItem | null>(novel);
  ...
  const totalChapterCount = useMemo(() => draft?.chapters.length ?? 0, [draft]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 px-4 py-6">
      <div className="max-h-[95vh] w-full max-w-7xl overflow-hidden rounded-[1.75rem] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_30px_80px_rgba(15,23,42,0.28)]">
        <div className="flex items-center justify-between gap-4 border-b border-[var(--color-border)] px-5 py-4">
          <div>
            <p className="text-sm font-semibold text-[var(--color-brand-strong)]">Edit Novel</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-tight">{draft.title}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-[var(--color-muted)]">
                    เพิ่ม ลบ และแก้ไขตอนของเรื่องนี้ได้ใน modal เดียว
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    ...
                    setDraft({
                      ...draft,
                      chapterCount: totalChapterCount + 1,
                      chapters: [
                        ...draft.chapters,
                        {
                          id: `draft-${Date.now()}`,
                ...
                <div className="rounded-[1.25rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-5">
                  {activeChapter ? (
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--color-border)] pb-4">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-[var(--color-muted)]">
                            แก้ข้อมูลตอนและเขียนเนื้อหาแบบเต็มจอในฝั่งขวา
                          </p>
                        </div>
                      ...
                      <div className="rounded-[1rem] border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-4 py-3 text-sm text-[var(--color-muted)]">
                        โหมดนี้เป็น editor แบบเบาเพื่อให้เขียนและแก้ตอนใน modal ได้ทันที
                      </div>
                    </div>
                  ) : (
